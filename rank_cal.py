...
df['one_year_return_rank'] = df['one_year_return'].rank(ascending=False, method='min')
df['six_month_return_rank'] = df['six_month_return'].rank(ascending=False, method='min')
df['three_month_return_rank'] = df['three_month_return'].rank(ascending=False, method='min')
...
df['sharpe_ratio_rank'] = df['sharpe_ratio'].rank(ascending=False, method='min')
...
# 输出排名结果
# print(df[['代码', '名称 (A)', '综合得分']].to_string(index=False))
